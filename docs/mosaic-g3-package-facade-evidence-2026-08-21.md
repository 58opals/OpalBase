...
- OpalBase implementation: `e52f667ad0b58f557dc0f1b8c689e878312d7e25`
...
- A recovered `SessionOwner` also retains the exact Base wallet-recovery authority, but it fails closed until that same owner has returned package-authenticated terminal Fusion evidence. Exact journal-erasure authorization remains unavailable until the owner subsequently observes a terminal Base wallet disposition, so a nonterminal Fusion snapshot cannot release its wallet reservation.
...
- `swift build` succeeded on the same SwiftPM lane used for validation. `swift test` then passed 37 network-target tests in 10 suites and 1,048 local tests in 112 suites with zero failures; live-network flags were not enabled.
- The application facade passed 3/3 at the exact implementation revision. Its end-to-end case directly observed that the wallet journal remains empty while the initial Fusion recovery transition is persisted; rejected actor reentrancy during suspended durable persistence; persisted and published a discovery beacon; rejected wallet recovery from the resulting nonterminal restart snapshot; persisted and published an authenticated timeout abort; recovered the exact terminal protocol evidence across another restart; rejected premature erasure; and only then returned exact wallet release and journal-erasure authority.
...
The full-session facade is intentionally a direct phase API rather than a generic command enum or parallel state machine. OpalFusion remains the only protocol-state owner; OpalBase adds one transition driver for persistence, publication, recovery directives, duplicate handling, and terminal evidence. Fresh application creation reuses that sole owner after its bootstrap persistence handshake and only then enters the existing journal-construction path; it does not add a second owner, state machine, or recovery format. Recovered wallet authority is retained inside that same actor rather than exposed as a second application capability, and two monotonic in-memory observations enforce protocol-terminal then wallet-terminal ordering while each package-owned journal remains the durable source of truth across restart. The repeated public methods preserve phase-specific call-site labels and one-use signing boundaries without duplicating transition logic. Legacy post-manifest owner aliases were removed instead of keeping two facade families.
