...
The internal Mosaic wallet-host alpha is limited to deterministic reservation for two explicitly selected pairs: Opal v0 on chipnet and the frozen mainnet-alpha.4 profile on mainnet. Mainnet acceptance requires the exact `Mosaic/0-opal-mainnet-alpha.4` and `bch-mainnet-p2pkh-schnorr/0-opal-mainnet-alpha.4` identifier pair, so source-compatible OpalFusion profile drift fails before wallet or recovered-broadcast mutation. The host provides one actor-owned lifecycle, exact-operation pinning, deadline- and cancellation-gated contributor signing, complete-transaction signature verification, and write-ahead recovery. One internal store seals each complete versioned journal snapshot with AES-GCM under a caller-owned key and wallet/journal scope, exclusively creates the empty marker before fresh authority exists, and returns only authenticated recovery authority after restart; a zero-record marker is uncertain and cannot be reused as a fresh attempt. The startup gate re-quarantines matching inputs and withholds broadcast authority when a journaled input is missing or changed. The broadcast coordinator accepts only an exact committed or recovered candidate plus a transaction client whose network identity derives from its concrete Fulcrum configuration, then requires a broadcasting-enabled security profile, explicit app approval, and durable approval and intent records before I/O. Mainnet coverage remains synthetic: the app still owns key protection, atomic durable file replacement, journal enumeration, cross-process exclusion, rollback/deletion detection, recovery execution, and user policy, while public session composition, concrete Tor/relay deployment, missing-input tombstones, chain reconciliation, and actual mainnet broadcast remain unavailable. This must not be presented as live Mosaic support.
...
- Internal Mosaic reservation for the explicit Opal-v0/chipnet and mainnet-alpha.4/mainnet pairs, exact version binding, authoritative previous-output resolution, contributor signing, complete P2PKH Schnorr verification, AES-GCM-authenticated whole-journal snapshots, one-use fresh and recovery capabilities, exact-input startup quarantine, and approval-gated exact-byte broadcast coordination through a Fulcrum-derived network capability on macOS; the proof remains synthetic, while app durability/rollback policy, recovery executors, public session wiring, concrete private transport, and actual broadcast remain deferred.
