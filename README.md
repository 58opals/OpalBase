...
- Internal Mosaic reservation for the explicit Opal-v0/chipnet and mainnet-alpha.3/mainnet pairs, alpha.3 local contribution-share validation, exact previous-output and final-transaction fee validation, contributor signing, complete P2PKH Schnorr transaction verification, recovery-journal seam, and exact-byte broadcast coordination on macOS; the six-contributor mainnet proof remains synthetic, while durable storage, production network composition, protocol transport, broadcast permission, and public runtime wiring remain deferred.
