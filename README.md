...
The internal Mosaic wallet-host alpha is limited to deterministic, in-process reservation for two explicitly selected pairs: Opal v0 on chipnet and the frozen mainnet-alpha.4 profile on mainnet. Mainnet acceptance requires the exact `Mosaic/0-opal-mainnet-alpha.4` and `bch-mainnet-p2pkh-schnorr/0-opal-mainnet-alpha.4` identifier pair, so a source-compatible OpalFusion profile revision fails before wallet or recovered-broadcast mutation. Alpha.4 preserves the roster-derived fee share and exact local input/output value equation introduced by alpha.3 while rejecting legacy alpha.3 transaction-profile identifiers before wallet mutation; OpalFusion remains responsible for deriving that share from the roster, and Opal-v0 component-fee behavior is unchanged. The host provides actor-safe exact-operation pinning, deadline- and cancellation-gated contributor signing, complete-transaction signature verification, and write-ahead recovery planning. Given app-loaded integrity-checked journal records, an internal startup gate re-quarantines every matching attempt input and withholds broadcast authority when any journaled input is missing or has changed. The broadcast coordinator accepts only an exact host-committed or gate-recovered candidate, requires a broadcasting-enabled security profile and explicit app approval, and durably records that approval separately from the broadcast intent before network I/O. The authoritative transaction-reader adapter can resolve exact previous-output facts for OpalFusion, while the existing signing policy independently refetches and validates them. Mainnet coverage remains synthetic and in memory; there is no public runnable session composition, durable journal codec or loader, missing-input tombstone or chain-reconciliation executor, live transport, production network-identity binding, recovery executor, or mainnet broadcast, and this must not be presented as live Mosaic support.
...
- Internal Mosaic reservation for the explicit Opal-v0/chipnet and mainnet-alpha.4/mainnet pairs, alpha.4 local contribution-share validation and legacy-alpha.3 identifier rejection, authoritative previous-output resolution with independent signing-time revalidation, exact final-transaction fee validation, contributor signing, complete P2PKH Schnorr transaction verification, recovery-journal planning, exact-input startup quarantine with missing-input reconciliation blocking, and distinctly recorded approval-gated exact-byte broadcast coordination on macOS; the mainnet proof remains synthetic, while public runnable session composition, durable journal loading, output and chain reconciliation, protocol transport, production network identity, actual broadcast, and public session wiring remain deferred.
