...
@Suite("Network Gateway Client", .tags(.unit, .network, .transaction))
struct NetworkGatewayClientSuite {
    @Test("caches acknowledged transactions after broadcast", .tags(.unit, .network, .transaction))
    func cachesAcknowledgedTransactionsAfterBroadcast() async throws {
    ...
    @Test("resolves already known transactions from broadcast errors", .tags(.unit, .network, .transaction))
    func resolvesAlreadyKnownTransactionsFromBroadcastErrors() async throws {
    ...
    @Test("normalizes transport errors during transaction fetch", .tags(.unit, .network, .transaction))
    func normalizesTransportErrorsDuringFetch() async throws {
        ...
        let client = MockGatewayClient(
            currentMempool: [],
            broadcast: { _ in Transaction.Hash(naturalOrder: Data(repeating: 0x01, count: 32)) },
