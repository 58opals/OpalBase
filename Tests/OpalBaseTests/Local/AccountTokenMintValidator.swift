...
struct AccountTokenMintValidator {
    ...
    func usesAuthorityInputWhenMintingNonFungibleTokens() async throws {
        ...
        let category = try OpalBase.CashTokens.CategoryIDModel(transactionOrderData: Data(repeating: 0xC1, count: 32))
        ...
        let recipients = [
            try OpalBase.Account.TokenMint.Recipient(
                ...
                nft: OpalBase.CashTokens.NFTModel(capability: .none, commitment: Data([0x02]))
            ),
            try OpalBase.Account.TokenMint.Recipient(
                address: recipientAddress,
                nft: OpalBase.CashTokens.NFTModel(capability: .none, commitment: Data([0x03]))
            )
        ]
    ...
    func preservesFungibleTokensOnChangeWhenAuthorityReturnsExternally() async throws {
        let account = try await makeAccount()
        let category = try OpalBase.CashTokens.CategoryIDModel(transactionOrderData: Data(repeating: 0xC2, count: 32))
        let mintingNonFungibleToken = try OpalBase.CashTokens.NFTModel(capability: .minting, commitment: Data([0x04]))
        ...
        let recipients = [
            try OpalBase.Account.TokenMint.Recipient(
                address: recipientAddress,
                nft: OpalBase.CashTokens.NFTModel(capability: .none, commitment: Data([0x05]))
            )
        ]
