...
@Suite("Network+Wallet+FeeRate")
struct NetworkWalletFeeRateTests {
    @Test("Restores persisted snapshots on first fetch")
    func restoreFromPersistence() async throws {
        let persistence = InMemoryFeePersistence()
        await persistence.prime(tier: .fast, value: 142, timestamp: Date(), version: 3)

        let subject = Network.Wallet.FeeRate(rateProvider: { _ in
            throw FixtureError.unexpectedFetch
        },
        cacheThreshold: 60,
        smoothingAlpha: 0,
    ...
    @Test("Invalidates stale caches on gateway events")
    func invalidatesOnGatewayEvents() async throws {
        let persistence = InMemoryFeePersistence()
        let provider = SequencedRateProvider(values: [110, 210])

        let subject = Network.Wallet.FeeRate(rateProvider: { tier in
            try await provider.next(for: tier)
        },
        cacheThreshold: 1,
        smoothingAlpha: 0,
    ...
    @Test("Ignores stale snapshots beyond the persistence window")
    func ignoresStaleSnapshots() async throws {
        let persistence = InMemoryFeePersistence()
        let provider = SequencedRateProvider(values: [330])

        await persistence.prime(tier: .fast,
                                value: 25,
...
private actor InMemoryFeePersistence {
    ...
    var adapter: Network.Wallet.FeeRate.Persistence {
        let actor = self
        return .init(loader: { tier, maxAge in
            try await actor.load(tier: tier, maxAge: maxAge)
        },
        writer: { tier, value, _, expected in
            try await actor.store(tier: tier, value: value, expectedVersion: expected)
        },
        remover: { tier, expected in
            try await actor.remove(tier: tier, expectedVersion: expected)
        })
    }

    func prime(tier: Network.Wallet.FeeRate.Tier,
               value: UInt64,
               timestamp: Date,
               version: UInt64) {
    ...
    func load(tier: Network.Wallet.FeeRate.Tier,
              maxAge: TimeInterval) throws -> Network.Wallet.FeeRate.Persistence.Snapshot? {
    ...
    func store(tier: Network.Wallet.FeeRate.Tier,
               value: UInt64,
               expectedVersion: UInt64?) throws -> Network.Wallet.FeeRate.Persistence.Snapshot {
        let now = Date()
        if var existing = storage[tier] {
            if let expectedVersion, existing.version != expectedVersion {
                throw Network.Wallet.FeeRate.Persistence.Error.conflict(expected: expectedVersion,
    ...
    func remove(tier: Network.Wallet.FeeRate.Tier,
                expectedVersion: UInt64?) throws {
