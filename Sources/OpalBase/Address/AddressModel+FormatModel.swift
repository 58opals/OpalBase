...
public struct AddressModel {
    ...
    public let lockingScript: ScriptModel
    ...
    public init(string: String) throws {
        if string.contains(AddressModel.separator) {
            self = try AddressModel.parseCashAddress(from: string)
            return
        }
        
        if let cashAddress = try? AddressModel.parseCashAddress(from: string) {
            self = cashAddress
            return
        }
        
        if let legacyAddress = try? AddressModel.parseLegacyAddress(from: string) {
            self = legacyAddress
            return
        }
    ...
    public init(script: ScriptModel, format: FormatModel = .standard) throws {
        let string = try AddressModel.makeCashAddressString(for: script, format: format)
        self.init(cashAddressPayload: string, lockingScript: script, format: format)
    }
    
    init(cashAddressPayload: String, lockingScript: ScriptModel, format: FormatModel) {
...
extension AddressModel {
    ...
    public var tokenAwareString: String {
        (try? AddressModel.makeCashAddressString(for: lockingScript, format: .tokenAware)) ?? string
    }
    ...
    static func generateChecksum(prefix: String, payload5BitValues: [UInt8]) throws -> [UInt8] {
        var values = try AddressModel.convertPrefixToFiveBitValues(prefix: prefix) + [0x00]
    ...
    private static func makeCashAddressString(for script: ScriptModel, format: FormatModel) throws -> String {
        ...
        case .p2sh(let scriptHash):
            guard scriptHash.count == 20 else { throw AddressModel.LegacyModel.Error.invalidScriptType }
            payload = Data([versionByte]) + scriptHash
        default:
            throw AddressModel.LegacyModel.Error.invalidScriptType
        }
...
extension AddressModel: CustomStringConvertible {
    ...
    public func generateString(withPrefix: Bool = false) -> String {
        withPrefix ? (AddressModel.prefix + AddressModel.separator + string) : string
    }
}

extension AddressModel: Hashable {
    public static func == (lhs: AddressModel, rhs: AddressModel) -> Bool {
