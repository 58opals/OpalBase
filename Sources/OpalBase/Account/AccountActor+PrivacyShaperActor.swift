// AccountActor+PrivacyShaperActor.swift
...
extension AccountActor.PrivacyShaperActor {
    ...
    func applyCoinSelectionHeuristics(to utxos: [TransactionModel.OutputModel.UnspentModel]) -> [TransactionModel.OutputModel.UnspentModel] {
        guard configuration.shouldRandomizeUTXOOrdering, utxos.count > 1 else { return utxos }
        return utxos.shuffled(using: &generator)
    }
    
    func organizeOutputs(_ outputs: [TransactionModel.OutputModel]) -> [TransactionModel.OutputModel] {
        ...
        return TransactionModel.OutputModel.applyBIP69Ordering(outputs)
    }
