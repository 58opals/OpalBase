// AccountActor~TokenGenesisPlanning.swift

import Foundation

extension AccountActor {
    public func prepareTokenGenesis(
        _ genesis: TokenGenesisModel,
        preferredGenesisInput: TransactionModel.OutputModel.UnspentModel? = nil,
        feePolicy: WalletActor.FeePolicy = .init()
    ) async throws -> TokenGenesisPlanModel {
        ...
        let genesisInput: TransactionModel.OutputModel.UnspentModel
        ...
        do {
            category = try CashTokensModel.CategoryIDModel(transactionOrderData: genesisInput.previousTransactionHash.naturalOrder)
        } catch {
            throw Error.tokenGenesisInvalidGenesisInput
        }
        
        let changeEntry = try await addressBook.selectNextEntry(for: .change)
        var rawOutputs: [TransactionModel.OutputModel] = .init()
        for recipient in genesis.recipients {
            let tokenData = CashTokensModel.TokenData(category: category,
        ...
        if let reservedSupply = genesis.reservedSupplyToSelf {
            let tokenChangeAddress = try AddressModel(script: changeEntry.address.lockingScript,
                                                 format: .tokenAware)
            let mintingToken: CashTokensModel.NFTModel?
            if reservedSupply.shouldIncludeMintingNonFungibleToken {
                do {
                    mintingToken = try CashTokensModel.NFTModel(capability: .minting,
        ...
        return TokenGenesisPlanModel(genesis: genesis,
                                category: category,
                                feeRate: feeRate,
