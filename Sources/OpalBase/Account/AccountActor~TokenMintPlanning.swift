...
extension AccountActor {
    public func prepareTokenMint(
        ...
        var extraFungibleInputs: [TransactionModel.OutputModel.UnspentModel] = .init()
        ...
        let tokenRecipientOutputs = try mint.recipients.map { recipient in
            let tokenData = CashTokensModel.TokenData(category: mint.category,
        ...
        var authorityReturnOutput: TransactionModel.OutputModel?
        var fungiblePreservationOutput: TransactionModel.OutputModel?
        switch mint.authorityReturn {
        case .toWalletChange:
            
            let authorityToken = CashTokensModel.TokenData(category: mint.category,
        ...
        case .toAddress(let address, let bitcoinCashAmount):
            ...
            if preservedFungible > 0 {
                let fungibleToken = CashTokensModel.TokenData(category: mint.category,
        ...
        return TokenMintPlanModel(mint: mint,
                             feeRate: feeRate,
