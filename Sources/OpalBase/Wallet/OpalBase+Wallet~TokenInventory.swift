...
extension _OpalBase.Wallet {
    public func loadUnspentOutputBalances() async throws -> OpalBase.Address.Book.UnspentOutputBalancesModel {
        ...
        var nonFungibleTokensByGroup: [OpalBase.Address.Book.TokenInventoryModel.NonFungibleTokenGroup: Int] = .init()
        ...
        let tokenInventory = OpalBase.Address.Book.TokenInventoryModel(fungibleAmountsByCategory: fungibleAmountsByCategory,
                                                         nonFungibleTokensByGroup: nonFungibleTokensByGroup)
        return OpalBase.Address.Book.UnspentOutputBalancesModel(bchTotal: bchTotal,
...
private extension _OpalBase.Wallet {
    func mergeFungibleAmounts(from additions: [OpalBase.CashTokens.CategoryIDModel: UInt64],
                              into totals: inout [OpalBase.CashTokens.CategoryIDModel: UInt64]) throws {
    ...
    func mergeNonFungibleTokens(from additions: [OpalBase.Address.Book.TokenInventoryModel.NonFungibleTokenGroup: Int],
                                into totals: inout [OpalBase.Address.Book.TokenInventoryModel.NonFungibleTokenGroup: Int]) {
