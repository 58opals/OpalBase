// WalletActor+FulcrumAddressActor+MonitorActor~Subscription.swift

import Foundation

extension WalletActor.FulcrumAddressActor.MonitorActor {
    func registerEntry(_ entry: AddressModel.BookActor.EntryModel) {
    ...
    private func ensureSubscription(for address: AddressModel) {
    ...
    private func consumeSubscription(stream: AsyncThrowingStream<NetworkModel.AddressSubscriptionUpdateModel, any Swift.Error>,
                                     address: AddressModel) async throws {
        do {
    ...
    private func handleAddressUpdate(for address: AddressModel) async {
        do {
    ...
    private func handleIncrementalFailure(for address: AddressModel, error: Swift.Error) async {
        if error.isCancellationError { return }
        await publishFailure(address: address, error: error)
