...
extension _OpalBase.CashTokens.TokenData {
    init(swiftFulcrumTokenData: SwiftFulcrum.RPC.Method.Blockchain.CashTokens.JSON) throws {
        let category = try OpalBase.CashTokens.CategoryIDModel(hexFromRPC: swiftFulcrumTokenData.category)
        let amount = try Self.parseAmount(from: swiftFulcrumTokenData.amount)
        let nft = try swiftFulcrumTokenData.nft.map { try OpalBase.CashTokens.NFTModel(swiftFulcrumNFT: $0) }
...
private extension _OpalBase.CashTokens.NFTModel {
    init(swiftFulcrumNFT: SwiftFulcrum.RPC.Method.Blockchain.CashTokens.JSON.NFT) throws {
        let capability = OpalBase.CashTokens.NFTModel.Capability(swiftFulcrumCapability: swiftFulcrumNFT.capability)
        let commitment: Data
        do {
...
private extension _OpalBase.CashTokens.NFTModel.Capability {
