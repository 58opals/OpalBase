...
extension _OpalBase.Transaction.OutputModel {
    ...
    static func applyBIP69Ordering(_ outputs: [OpalBase.Transaction.OutputModel]) -> [OpalBase.Transaction.OutputModel] {
    ...
    private static func compareNonFungibleToken(lhs: OpalBase.CashTokens.NFTModel?, rhs: OpalBase.CashTokens.NFTModel?) -> Bool {
        switch (lhs, rhs) {
        case (nil, nil):
            return false
        case (nil, _):
    ...
    private static func rankCapability(_ capability: OpalBase.CashTokens.NFTModel.Capability) -> Int {
        switch capability {
        case .none:
            return 0
        case .mutable:
