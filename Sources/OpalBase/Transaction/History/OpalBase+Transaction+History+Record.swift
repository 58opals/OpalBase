// OpalBase+Transaction+History+Record.swift
...
extension _OpalBase.Transaction.History {
    public struct Record: Sendable, Hashable, Equatable {
        ...
        public struct VerificationMetadata: Sendable, Hashable, Equatable {
            ...
            mutating func synchronize(with recordStatus: OpalBase.Transaction.History.Status,
                                      timestamp: Date,
...
extension _OpalBase.Transaction.History.Record {
    ...
    static func makeRecord(for entry: OpalBase.Transaction.History.Entry,
                           scriptHash: String,
        ...
        let verificationStatus: OpalBase.Transaction.History.Status.Verification = statusTransition.isConfirmed ? .pending : .unknown
        let chainMetadata = OpalBase.Transaction.History.Record.ChainMetadata(height: entry.height,
                                                                     fee: entry.fee,
                                                                     scriptHashes: [scriptHash],
                                                                     firstSeenAt: timestamp,
                                                                     lastUpdatedAt: timestamp)
        let confirmationMetadata = OpalBase.Transaction.History.Record.ConfirmationMetadata(height: confirmationHeight,
                                                                                   confirmedAt: confirmedAt)
        let verificationMetadata = OpalBase.Transaction.History.Record.VerificationMetadata(status: verificationStatus,
                                                                                   merkleProof: nil,
                                                                                   lastVerifiedHeight: nil,
                                                                                   lastCheckedAt: nil)
        return OpalBase.Transaction.History.Record(transactionHash: entry.transactionHash,
    ...
    mutating func updateVerification(status: OpalBase.Transaction.History.Status.Verification,
    ...
    private mutating func applyEntryDetails(from entry: OpalBase.Transaction.History.Entry,
                                            scriptHash: String,
                                            timestamp: Date) {
    ...
    private mutating func applyStatusTransition(_ transition: OpalBase.Transaction.History.Status.Transition,
                                                entryHeight: Int,
                                                timestamp: Date) {
