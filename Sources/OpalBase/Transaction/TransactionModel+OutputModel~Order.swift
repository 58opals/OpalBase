// TransactionModel+OutputModel~Order.swift
...
extension TransactionModel.OutputModel {
    ...
    static func applyBIP69Ordering(_ outputs: [TransactionModel.OutputModel]) -> [TransactionModel.OutputModel] {
    ...
    private static func compareTokenData(lhs: CashTokensModel.TokenData?, rhs: CashTokensModel.TokenData?) -> Bool {
        switch (lhs, rhs) {
        case (nil, nil):
            return false
        case (nil, _):
    ...
    private static func compareNonFungibleToken(lhs: CashTokensModel.NFTModel?, rhs: CashTokensModel.NFTModel?) -> Bool {
        switch (lhs, rhs) {
        case (nil, nil):
            return false
        case (nil, _):
    ...
    private static func rankCapability(_ capability: CashTokensModel.NFTModel.Capability) -> Int {
        switch capability {
        case .none:
            return 0
        case .mutable:
