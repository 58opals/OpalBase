// UInt256Model~Arithmetic.swift

import Foundation

extension UInt256Model {
    @inlinable
    func add(_ other: UInt256Model) -> (sum: UInt256Model, carry: Bool) {
        ...
        return (UInt256Model(limbs: result), carry != 0)
    }

    @inlinable
    func subtract(_ other: UInt256Model) -> (difference: UInt256Model, borrow: Bool) {
        var result: InlineArray<4, UInt64> = .init(repeating: 0)
        var borrow: UInt64 = 0
        for index in 0..<4 {
        ...
        return (UInt256Model(limbs: result), borrow != 0)
    }

    @inlinable
    func multiplyFullWidth(by other: UInt256Model) -> UInt512Model {
        ...
        for leftIndex in 0..<4 {
            ...
            for rightIndex in 0..<4 {
                ...
                let (sum, newCarry) = UInt256Model.multiplyAdd(
                    low: low,
        ...
        return UInt512Model(limbs: result)
    }
    ...
    @inlinable
    func squareFullWidth() -> UInt512Model {
        var result: InlineArray<8, UInt64> = .init(repeating: 0)
