// LargeUnsignedIntegerModel.swift
...
struct LargeUnsignedIntegerModel: Comparable, Sendable {
    ...
    func shiftLeft(by bits: Int) -> LargeUnsignedIntegerModel {
        ...
        return LargeUnsignedIntegerModel(data)
    }
    
    func shiftRight(by bits: Int) -> LargeUnsignedIntegerModel {
        guard bits > 0 else { return self }
        precondition(bits % 8 == 0, "Shift must be a multiple of 8.")
        var data = serialize()
    ...
    public static func < (lhs: LargeUnsignedIntegerModel, rhs: LargeUnsignedIntegerModel) -> Bool {
    ...
    mutating func divide(by divisor: Int) -> Int {
        ...
        self = LargeUnsignedIntegerModel(words: quotientWords)
