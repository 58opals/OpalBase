...
enum CompactSizeModel {
    case uint8(UInt8)
    case uint16(UInt16)
    case uint32(UInt32)
    case uint64(UInt64)
    
    /// Initializes a CompactSizeModel instance from a UInt64 value.
    ...
    /// Initializes a CompactSizeModel instance from Data.
    /// - Parameter data: The data to initialize from.
    /// - Throws: `CompactSizeModel.Error` if the data is insufficient or the prefix is invalid.
    ...
    /// Encodes the CompactSizeModel value into Data.
    ...
    /// Decodes a CompactSizeModel instance from Data.
    /// - Parameter data: The data to decode from.
    /// - Throws: `CompactSizeModel.Error` if decoding fails.
    /// - Returns: A tuple containing the decoded CompactSizeModel and the number of bytes read.
    static func decode(from data: Data) throws -> (CompactSizeModel, Int) {
        let compactSize = try CompactSizeModel(data: data)
        let bytesRead = compactSize.encodedSize
        
        return (compactSize, bytesRead)
    }
    
    /// Returns the UInt64 representation of the CompactSizeModel.
    var value: UInt64 {
        switch self {
