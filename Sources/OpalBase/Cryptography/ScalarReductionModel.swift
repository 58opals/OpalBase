...
enum ScalarReductionModel {
    @usableFromInline
    static let reductionConstant = UInt256Model(
        limbs: [
            0x402da1732fc9bebf,
            0x4551231950b75fc4,
            0x0000000000000001,
    ...
    @inlinable
    static func reduce(_ value: UInt512Model) -> UInt256Model {
        var foldedValue = value
        while true {
            let highPart = UInt256Model(limbs: [
                foldedValue.limbs[4],
                foldedValue.limbs[5],
                foldedValue.limbs[6],
                foldedValue.limbs[7]
            ...
            if highPart.isZero {
                ...
                if result.compare(to: OpalBase.Cryptography.Secp256k1.Constant.n) != .orderedAscending {
                    result = result.subtract(OpalBase.Cryptography.Secp256k1.Constant.n).difference
                }
                return result
            }
            
            let lowPart = UInt256Model(limbs: [
                foldedValue.limbs[0],
                foldedValue.limbs[1],
                foldedValue.limbs[2],
                foldedValue.limbs[3]
    ...
    @inlinable
    static func addLow256(_ value: inout UInt512Model, _ lowPart: UInt256Model) {
        let currentLow = UInt256Model(limbs: [
            value.limbs[0],
            value.limbs[1],
            value.limbs[2],
            value.limbs[3]
