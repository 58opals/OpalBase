...
extension NonceGenerator {
    struct RFC6979 {
        ...
        mutating func makeNextScalar() throws -> Scalar {
            while true {
                valueBytes = NonceGenerator.makeAuthenticationCode(key: keyBytes, message: valueBytes)
        ...
        private mutating func updateKeyAndValue(separator: UInt8, includeKeyMaterial: Bool) {
            ...
            keyBytes = NonceGenerator.makeAuthenticationCode(key: keyBytes, message: material)
            valueBytes = NonceGenerator.makeAuthenticationCode(key: keyBytes, message: valueBytes)
        }
    }
}
