...
struct ScalarModel: Sendable, Equatable {
    ...
    init(data32: Data, requireNonZero: Bool = false) throws {
        ...
        guard parsed.compare(to: OpalBase.Cryptography.Secp256k1.Constant.n) == .orderedAscending else {
    ...
    init(value: UInt256Model) throws {
        guard value.compare(to: OpalBase.Cryptography.Secp256k1.Constant.n) == .orderedAscending else {
            throw Error.invalidScalarValue
        }
        self.value = value
    }
    ...
    func addModN(_ other: ScalarModel) -> ScalarModel {
        ...
        var reduced = sum
        if carry || reduced.compare(to: OpalBase.Cryptography.Secp256k1.Constant.n) != .orderedAscending {
    ...
    func subModN(_ other: ScalarModel) -> ScalarModel {
        ...
        if borrow {
            reduced = reduced.add(OpalBase.Cryptography.Secp256k1.Constant.n).sum
        }
        return ScalarModel(unchecked: reduced)
    }
    ...
    func negateModN() -> ScalarModel {
        ...
        let difference = OpalBase.Cryptography.Secp256k1.Constant.n.subtract(value).difference
