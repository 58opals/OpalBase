// ClaimableSpendPathResolution.swift

import Foundation

enum ClaimableSpendPathResolution {
    private static let schnorrSignatureWithHashTypeByteCount = 65
    private static let compressedPublicKeyByteCount = 33

    static func makeSpendPath(
        ...
        guard let signatureWithHashType = try? readPushedElement(from: bytes, offset: &offset),
              signatureWithHashType.count == schnorrSignatureWithHashTypeByteCount,
              let compressedPublicKey = try? readPushedElement(from: bytes, offset: &offset),
              compressedPublicKey.count == compressedPublicKeyByteCount,
              offset < bytes.count
        else {
            return nil
        }
        ...
        guard let redeemScriptData = try? readPushedElement(from: bytes, offset: &offset),
