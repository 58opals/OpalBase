// OpalBase+Claimable+Primitives.swift

import Foundation
import OpalCrypto

func makeClaimableCompressedPublicKey(
...
func makeClaimablePublicKeyHash(
    ...
    OpalCryptoAdapter.hash160(
        try makeClaimableCompressedPublicKey(from: privateKey, invalidError: invalidError)
    )
}

func makeClaimableQueryScriptHashHex(from lockingScript: Data) -> String {
    OpalCryptoAdapter.sha256(lockingScript).reversedData.hexadecimalString
}

func makeClaimableWalletImportFormat(
    privateKey: Data,
    ...
    var payload = Data([makeClaimableWalletImportFormatVersion(for: network)])
...
private func makeClaimableWalletImportFormatVersion(
...
func makeClaimableScriptNumberOperationData(for value: UInt32) -> Data {
    switch value {
    case 0:
