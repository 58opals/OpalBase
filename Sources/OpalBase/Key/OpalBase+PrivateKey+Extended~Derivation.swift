// OpalBase+PrivateKey+Extended~Derivation.swift

import Foundation

extension _OpalBase.PrivateKey.Extended {
    private func deriveChildPrivateKey(at index: UInt32, computeParentFingerprint: Bool) throws -> OpalBase.PrivateKey.Extended {
        ...
        do {
            childPrivateKey = try OpalBase.Cryptography.Secp256k1.Operation.tweakAddPrivateKey32(parentPrivateKey, tweak32: leftHashMessageAuthenticationCodePart)
        } catch {
    ...
    func deriveNonHardenedChildUsingParentKey(at index: UInt32,
                                              ...
                                              parentFingerprint: Data) throws -> OpalBase.PrivateKey.Extended {
        guard !Harden.checkHardened(index) else { throw OpalBase.PrivateKey.Error.invalidDerivedKey }
        ...
        let childPrivateKey: Data
        do {
            childPrivateKey = try OpalBase.Cryptography.Secp256k1.Operation.tweakAddPrivateKey32(privateKey, tweak32: leftHMACPart)
        } catch {
            throw OpalBase.PrivateKey.Error.invalidDerivedKey
        }
    ...
    func deriveChild(at path: OpalBase.DerivationPath) throws -> OpalBase.PrivateKey.Extended {
        try deriveChild(at: path.makeIndices())
    }

    func deriveChild(at indices: [UInt32]) throws -> OpalBase.PrivateKey.Extended {
    ...
    func deriveChildFast(at indices: [UInt32]) throws -> OpalBase.PrivateKey.Extended {
        var extendedKey = self
        let startingDepth = Int(depth)
