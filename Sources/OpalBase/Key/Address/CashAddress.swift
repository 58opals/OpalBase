...
struct CashAddress {
    ...
    init(_ script: Script) throws {
        ...
        case .p2pkh(let hash):
            ...
            let payload5BitValues = CashAddress.payloadTo5BitValues(payload: payload)
            let checksum = try CashAddress.generateChecksum(prefix: prefix, payload5BitValues: payload5BitValues)
            let combined = payload5BitValues + checksum
            self.string = prefix + ":" + Base32.encode(Data(combined))
            
        default:
            throw Address.Error.invalidScriptType
        }
    }
}
...
extension CashAddress {
    ...
    private static func generateChecksum(prefix: String, payload5BitValues: [UInt8]) throws -> [UInt8] {
        var values = try CashAddress.prefixTo5BitValues(prefix: prefix) + [0x00]
...
extension CashAddress: Hashable {
    static func == (lhs: CashAddress, rhs: CashAddress) -> Bool {
