...
public struct MnemonicModel {
    ...
    public init(length: LengthModel = .long, passphrase: String = "") throws {
        let entropy = try MnemonicModel.generateEntropy(numberOfBits: length.numberOfBits)
        let mnemonicWords = try MnemonicModel.generateMnemonicWords(from: entropy)
        let seed = try MnemonicModel.generateSeed(from: mnemonicWords, passphrase: passphrase)
    ...
    public init(words: [String], passphrase: String = "") throws {
        ...
        self.words = words
        self.seed = try MnemonicModel.generateSeed(from: words, passphrase: passphrase)
        self.passphrase = passphrase
    }
    ...
    public init(
        words: [String],
        passphrase: String = "",
        wordList: MnemonicModel.WordListModel
    ) throws {
    ...
    public init(
        words: [String],
        passphrase: String = "",
        wordLists: [MnemonicModel.WordModel.LanguageModel: MnemonicModel.WordListModel]
    ) throws {
...
extension MnemonicModel: Hashable {
...
extension MnemonicModel: Sendable {}
