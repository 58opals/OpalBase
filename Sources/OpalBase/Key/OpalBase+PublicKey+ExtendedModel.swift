...
extension _OpalBase.PublicKey {
    struct ExtendedModel {
        ...
        init(extendedPrivateKey: OpalBase.PrivateKey.ExtendedModel) throws {
            self.publicKey = try OpalBase.PublicKey(privateKey: .init(data: extendedPrivateKey.privateKey)).compressedData
...
extension _OpalBase.PublicKey.ExtendedModel: Hashable {
    static func == (lhs: OpalBase.PublicKey.ExtendedModel, rhs: OpalBase.PublicKey.ExtendedModel) -> Bool {
...
extension _OpalBase.PublicKey.ExtendedModel: CustomDebugStringConvertible {
...
extension _OpalBase.PublicKey.ExtendedModel {
    private func deriveChildPublicKey(at index: UInt32) throws -> OpalBase.PublicKey.ExtendedModel {
        let isHardened = HardenModel.checkHardened(index)
        guard !isHardened else { throw OpalBase.PublicKey.Error.hardenedDerivation }
        ...
        do {
            childPublicKey = try OpalBase.Cryptography.Secp256k1.Operation.tweakAddPublicKey(publicKey,
                                                                       tweak32: leftHMACPart,
                                                                       format: .compressed)
        } catch {
            throw OpalBase.PublicKey.Error.publicKeyDerivationFailed
        }
    ...
    func deriveChild(at path: OpalBase.DerivationPath) throws -> OpalBase.PublicKey.ExtendedModel {
        try deriveChild(at: path.makeIndices())
    }
    
    func deriveChild(at indices: [UInt32]) throws -> OpalBase.PublicKey.ExtendedModel {
        var extendedKey = self
        let startingDepth = Int(self.depth)
        
        guard indices.count >= startingDepth else {
            throw OpalBase.PublicKey.Error.derivationPathTooShort
        }
        
        for index in indices.dropFirst(startingDepth) {
            guard !HardenModel.checkHardened(index) else { throw OpalBase.PublicKey.Error.hardenedDerivation }
...
extension _OpalBase.PublicKey.ExtendedModel: Sendable {}
extension _OpalBase.PublicKey.ExtendedModel: Equatable {}
