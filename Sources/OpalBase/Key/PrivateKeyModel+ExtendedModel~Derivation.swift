...
extension PrivateKeyModel.ExtendedModel {
    private func deriveChildPrivateKey(at index: UInt32, computeParentFingerprint: Bool) throws -> PrivateKeyModel.ExtendedModel {
        ...
        if shouldComputeParentPublicKey {
            parentPublicKey = try PublicKeyModel(privateKey: .init(data: parentPrivateKey)).compressedData
        } else {
            parentPublicKey = nil
        }
    ...
    func deriveNonHardenedChildUsingParentKey(at index: UInt32,
                                              ...
                                              parentFingerprint: Data) throws -> PrivateKeyModel.ExtendedModel {
        guard !HardenModel.checkHardened(index) else { throw PrivateKeyModel.Error.invalidDerivedKey }
    ...
    func deriveChild(at path: DerivationPathModel) throws -> PrivateKeyModel.ExtendedModel {
        try deriveChild(at: path.makeIndices())
    }

    func deriveChild(at indices: [UInt32]) throws -> PrivateKeyModel.ExtendedModel {
        var extendedKey = self
        let startingDepth = Int(depth)

        guard indices.count >= startingDepth else {
            throw PrivateKeyModel.Error.derivationPathTooShort
        }
    ...
    func deriveChildFast(at indices: [UInt32]) throws -> PrivateKeyModel.ExtendedModel {
        var extendedKey = self
        let startingDepth = Int(depth)
