// Adapter+SwiftFulcrum+Gateway.swift

import Foundation
import SwiftFulcrum

extension Adapter.SwiftFulcrum {
    public static func gatewayAPI(fulcrum: SwiftFulcrum.Fulcrum) -> Network.Gateway.API {
        GatewayAPIBuilder(fulcrum: fulcrum).api()
    }
}

private struct GatewayAPIBuilder: @unchecked Sendable {
    let fulcrum: SwiftFulcrum.Fulcrum
    
    func api() -> Network.Gateway.API {
        Network.Gateway.API(
            currentMempool: { [] },
            broadcast: { transaction in try await self.broadcast(transaction) },
            fetch: { hash in try await self.fetch(hash) },
            getRawTransaction: { hash in try await self.getRawTransaction(for: hash) },
            getDetailedTransaction: { hash in try await self.getDetailedTransaction(for: hash) },
            getEstimateFee: { targetBlocks in try await self.getEstimateFee(targetBlocks: targetBlocks) },
            getRelayFee: { try await self.getRelayFee() },
            getHeader: { height in try await self.getHeader(height: height) },
            pingHeadersTip: { try await self.pingHeadersTip() },
            interpretBroadcastError: { error, expectedHash in
                self.interpretBroadcastError(error, expectedHash: expectedHash)
            },
            normalize: { error, request in
                self.normalize(error: error, during: request)
            }
        )
    }
    
    private func broadcast(_ transaction: Transaction) async throws -> Transaction.Hash {
    ...
    private func fetch(_ hash: Transaction.Hash) async throws -> Transaction? {
        do {
    ...
    private func getRawTransaction(for hash: Transaction.Hash) async throws -> Data {
        let response = try await fulcrum.submit(
    ...
    private func getDetailedTransaction(for hash: Transaction.Hash) async throws -> Transaction.Detailed {
        let response = try await fulcrum.submit(
            method: .blockchain(.transaction(.get(transactionHash: hash.externallyUsedFormat.hexadecimalString, verbose: true))),
            responseType: Response.Result.Blockchain.Transaction.Get.self
        )
    ...
    private func getEstimateFee(targetBlocks: Int) async throws -> Satoshi {
        let response = try await fulcrum.submit(
    ...
    private func getRelayFee() async throws -> Satoshi {
        let response = try await fulcrum.submit(
    ...
    private func getHeader(height: UInt32) async throws -> Network.Gateway.HeaderPayload? {
        let response = try await fulcrum.submit(
    ...
    private func pingHeadersTip() async throws {
    ...
    private func interpretBroadcastError(
        _ error: Swift.Error,
    ...
    private func normalizedError(for client: Fulcrum.Error.Client) -> Network.Gateway.Error {
        ...
        case .urlNotFound:
            return gatewayError(
                .transport(description: "Fulcrum endpoint URL not found"),
                hint: .never
            )
        case .invalidURL(let urlString):
            if let resolvedURL = URL(string: urlString) {
                let description = "Fulcrum endpoint URL is invalid: \(resolvedURL.absoluteString)"
                return gatewayError(.transport(description: description), hint: .never)
            }
            return gatewayError(
                .transport(description: "Fulcrum endpoint URL is invalid"),
                hint: .never
            )
