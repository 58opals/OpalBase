// OpalBase+Network+Fulcrum+TransactionProofReader.swift
    ...
    public struct TransactionProofReader {
        ...
        public func fetchTransactionIdentifier(
            ...
            guard blockHeight > 0 else {
                throw OpalBase.Network.Error(
                    reason: .protocolViolation,
                    message: "Transaction position resolution requires a positive block height."
                )
            }
            ...
            return try await OpalBase.Network.performWithFailureTranslation {
                ...
                } else if !result.merkle.isEmpty {
                    throw OpalBase.Network.Error(
                        reason: .protocolViolation,
                        message: "Position resolution returned an unrequested merkle proof."
                    )
                }
...
private extension _OpalBase.Network.Fulcrum.TransactionProofReader {
    static func validateMerkleBranch(_ branch: [String]) throws {
        for (index, hashString) in branch.enumerated() {
            ...
            if let decodedByteCount = Data.unprefixedHexadecimalByteCount(hashString),
               decodedByteCount != OpalBase.Transaction.Hash.expectedByteCount {
                throw SwiftFulcrum.Client.Error.client(
                    .protocolMismatch(
                        "Merkle proof branch hash length at index \(index): expected \(OpalBase.Transaction.Hash.expectedByteCount) bytes, got \(decodedByteCount)"
                    )
                )
            }
